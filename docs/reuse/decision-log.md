...
## D020: Add Wallet-Funded Charges Before Payment Ingestion

Decision: Phase 7 creates immutable `billing.BillingCharge` records and a `billing_charge` ledger entry type for operator-triggered Wallet-funded activation and renewal. The workflow spends existing Wallet credit only, charges exactly the active subscription snapshot price, rejects partial Wallet balances, leaves overpayment as Wallet credit, and keeps manual uncharged billing-period actions available. Phase 7 does not create payments, M-PESA records, Paybill or Till callbacks, invoices, receipts, discounts, bundles, customer-specific prices, automatic wallet allocation, automatic renewal, suspension, RADIUS rows, PPPoE credentials, RouterOS calls, provisioning jobs, notifications, or customer portals.

Reason: SuperSurf needs a reviewed way to consume existing Wallet credit for service time before payment ingestion can fund wallets automatically. Keeping the charge, ledger debit, and billing period in one transaction preserves auditability while avoiding premature payment, invoice, or network behavior.

## D021: Add Canonical Payment Foundation Before Daraja Adapter

Decision: Phase 8 creates provider-neutral `Payment`, `PaymentAllocation`, `PaymentProviderProfile`, and `UnmatchedPaymentCase` records with fake-provider ingestion only. Matched account references credit the account-level Wallet, unmatched payments remain valid payment records, and no real M-PESA, Paybill, Till, STK Push, reconciliation import, invoice, receipt, automatic Wallet spending, automatic renewal, or network behavior is implemented.

Reason: SuperSurf needs idempotent, immutable payment storage and Wallet-credit semantics before any provider-specific Daraja adapter is allowed to call into the system. A fake provider lets operators and tests prove matching, allocation, audit privacy, permissions, and PostgreSQL locking without storing credentials or assuming unverified callback contracts.
