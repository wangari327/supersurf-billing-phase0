...
On 2026-07-18, the Owner completed the live Phase 9.1 acceptance at the approved commit. PostgreSQL, the broker, web, Caddy, readiness, and both public HTTPS hosts were healthy. Ingestion used exactly one active sandbox M-PESA Paybill profile whose configured identifier matched the identifier used for C2B URL registration and the successful simulator request.

During setup, a mistakenly selected STK sandbox identifier was detected before any canonical Payment existed. Its unused profile had no associated Payments, was safely deactivated, and the correct Paybill profile was synchronized. A stale simulator acceptance response from earlier attempts did not correspond to a request received by the VPS and created no callback event, Payment, link, or Wallet entry. Operators must treat a fresh provider request identifier and captured callback as delivery evidence and must not repeat a payment merely because the portal displays a stale response.

After the validation and confirmation URLs were re-registered for the exact configured Paybill sandbox identifier, Daraja acknowledged registration successfully. One controlled `CustomerPayBillOnline` transaction for `KSh1` with synthetic reference `SS000001` delivered immediately and created exactly one allocated canonical Payment, one full Wallet allocation, one system-sourced `payment_credit` ledger entry, and one callback-payment link. The subscriber moved from no Wallet and `KSh0` to `KSh1`. The subscriber had no services, and no BillingPeriod, BillingCharge, activation, renewal, Wallet spending, invoice, receipt, notification, reconciliation, or network action occurred.

C2B registration is identifier-specific. Environment configuration, the active provider profile, URL registration, and the simulator shortcode must all refer to the same Paybill identity; an STK Business Shortcode is not a Paybill ingestion identifier. The application's fail-closed profile-conflict and evidence-first behavior prevented incorrect accounting. Equivalent retries were not forced during the live test because PostgreSQL automation already covers idempotency and duplicate-credit prevention.
