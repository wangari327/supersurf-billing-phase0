...
Phase 9.1 sandbox Paybill processing uses `MPESA_PAYBILL_INGESTION_ENABLED` and `MPESA_PAYBILL_EXTERNAL_IDENTIFIER`. The enable flag defaults false and the identifier defaults empty. Enabling is valid only in public LAB with a 5-12 digit identifier and a valid callback token. Private LAB and production fail closed when the flag is true. The identifier is not a credential, but its configured value must remain in the mode-600 deployment environment and must not be printed or logged.
...
Real-world values such as public domain, emails, Paybill number, Till number, KRA PIN, and licence information must remain empty until supplied by SuperSurf. Phase 9.1 does not permit Paybill processing in production even if its enable flag is set.
