...
`billing.view_mpesacallbackpaymentlink` is granted only to Owner, Administrator, and Finance. Callback detail reveals a linked Payment only when the operator also has `billing.view_payment`. Payment detail reveals callback provenance only with `billing.view_mpesacallbackevent`. Support and Read Only may view canonical Payments but cannot infer whether a callback sourced them; NOC has neither payment nor callback visibility.
